...
def mimic_ucf101(frame_path, anno_dir):
    ...
    classes_list.sort() # ordina in ordine alfabetico (POTREBBE NON SERVIRE)
...
def parse_hmdb51_splits(level):
    ...
    # print(class_mapping)                                    # 0 diventa train / 1 validation (test) / 2 nulla (chiamato test)
    def line2rec(line):
...
def build_split_list(split, frame_info, shuffle=False):
    def build_set_list(set_list):
        ...
        for item in set_list:
            #print(item[0])
            #print(frame_info)
            #print(item[0] in frame_info)
            #print(frame_info[item[0]])
        ...
        # print(flow_list) # problema la lista è sempre vuota
...
if len(split_tp) > 1:
    for i, split in enumerate(split_tp):
        #print(split_tp[i]) # tutti i path di brush sono passati ma la funzione è vuota
        lists = build_split_list(split_tp[i], frame_info,
                                    shuffle=args.shuffle)
        # print(lists) lista vuota restituita dalla funzione (quindi file vuoto)
